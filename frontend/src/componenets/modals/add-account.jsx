...
const accounts = ["Cash", "Crypto", "Paypal", "Visa Debit Card","Venmo", "Zelle"];
...
const AddAccount = ({ isOpen, setIsOpen, refetch }) => {
  ...
  const [data, setData] = useState(null); // Defined state for account data
  const [isLoading, setIsLoading] = useState(false); // Defined loading state
  ...
  const fetchAccounts = async () => {
    try {
      ...
      console.log(res); // Check the API response
      setData(res?.data);
    } catch (error) {
      console.log(error);
